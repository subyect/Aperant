...
function getUtilitySettings(): { model: string; modelId: string; thinkingLevel: string; thinkingBudget: number | null } {
  ...
  try {
    if (existsSync(settingsPath)) {
      ...
      const model = normalizeOpenAISubscriptionModel(featureModels.utility || DEFAULT_FEATURE_MODELS.utility);
      const thinkingLevel = featureThinking.utility || DEFAULT_FEATURE_THINKING.utility;

      return {
        model,
        modelId: MODEL_ID_MAP[model as keyof typeof MODEL_ID_MAP] || model || MODEL_ID_MAP.haiku,
        thinkingLevel,
  ...
  return {
    ...
    modelId: MODEL_ID_MAP[DEFAULT_FEATURE_MODELS.utility as keyof typeof MODEL_ID_MAP] || DEFAULT_FEATURE_MODELS.utility,
